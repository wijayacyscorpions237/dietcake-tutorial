...
class SimpleDBI
{
    ...
    {
        return isset($this->exec_time) ? $this->exec_time : null;
    }
    ...
    public function search($table, $where, $params, $order = null, $limit = null, $options = array())
    {
        ...
        if (isset($options['force_index'])) {
            $force_index_query = sprintf(' FORCE INDEX (%s)', $options['force_index']);
        }

        // SQL 文の組み立て
        $sql = sprintf('SELECT %s FROM %s%s WHERE %s', $select_expr, $table, $force_index_query, $where);
